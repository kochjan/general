...
def all_comp_names(save2f=True):
    ...
    if save2f:
        all_df.to_csv('./data/all_comp_names.csv', index=None)
    return all_df


def all_dates(symb, save2f=True):#for one company, find all qtr ends
    ...
    if save2f:
        all_list_dates_df.to_csv('./data/'+symb+'_dates.csv', index=None)
...
def parse_new(r):
    ...
    for tr in trs[2:]:
        ...
        col9 = [i for i in s.findAll('tr') if 'as on' in i.text.lower()][-1].findAll('td')[-1].text
...
def downloader(all_list_dates_df, col_n, w2file = True, db = True, fromfilefirst=False):
...
if __name__ == '__main__':
    ...
    if opts.qtrend:#if give a specific quarter end date
        ...
        downloader(all_list_on_a_date_df, col_n, fromfilefirst=opts.fff)
    
    else:

        all_df = all_comp_names() # names for all companies
        for symb in all_df['symbol']:
            print "start %s --------------"%symb
            all_list_dates_df = all_dates(symb) # all (name, date) combination
            downloader(all_list_dates_df, col_n, fromfilefirst=opts.fff)
            print "end %s --------------"%symb
