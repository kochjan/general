...
base_url = 'http://dmz9.moea.gov.tw/gmweb/investigate/InvestigateEB.aspx?lang=E'
db_name = 'dwang..twn_IPTSRL_survey'
beg_month = '1990Q1'
period='Q'
...
beg_str = re.search('value="(.+)"', str(selectbeg.findAll('option')[-1])).groups()[0]
selectend = s.findAll('select', {'id':'ContentPlaceHolder1_ddlDateEnd'})[0]
end_str = re.search('value="(.+)"', str(selectend.findAll('option')[0])).groups()[0]
...
for item2n in item2n_dict:
    ...
    data = {'__VIEWSTATE':viewstate, '__EVENTVALIDATION':eventvalidation, 'ctl00$ContentPlaceHolder1$hidLanguage':'E', 'ctl00$ContentPlaceHolder1$ddlPeriod':period, 'ctl00$ContentPlaceHolder1$ddlDateBeg':beg_month if beg_month > beg_str else beg_str, 'ctl00$ContentPlaceHolder1$ddlDateEnd':end_str,\
...
item2n:'on'}
    ...
    col1=['value'] #this page is a special case
    res_df = pd.DataFrame(res_list, columns=['Year', 'Month']+col1)
    res_df['Industry'] = item2n_dict[item2n]
    res_df['Year'] = res_df['Year'].fillna()
    res_df['Year_Month'] = [i for i in map(lambda x: str(int(x)), res_df['Year'])+res_df['Month']]
    
    res_df = res_df.drop(['Year', 'Month'], axis=1)
    
    sql_io.write_frame(res_df, db_name, if_exists='append', bulk='off')
