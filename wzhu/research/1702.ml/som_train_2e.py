...
def run(date, only_country=None):
    ...
    for factor in SIGNALS + BARRA:
        ...
        data[factor + '.pos'] = data[factor+'.T'].apply(lambda x: 1 if x > 2 else 0)
        data[factor + '.neg'] = data[factor + '.T'].apply(lambda x: 1 if x < -2 else 0)
        data[factor + '.neut'] = data[factor + '.T'].apply(lambda x: 1 if np.abs(x) < 2 else 0)

    for sign in ['pos','neg','neut']:
        data['signal_'+sign] = data[ [x + '.' + sign for x in SIGNALS] ].apply(lambda x: reduce(np.add, x), axis=1)
        data['barra_' + sign] = data[[x + '.' + sign for x in BARRA]].apply(lambda x: reduce(np.add, x), axis=1)

    context_cols = [x for x in data.columns if ('signal_' in x) or ('barra_' in x)]
    ...
    for signal in SIGNALS:
        # use signal relevant history, and use context for BARRA factors and other signals
