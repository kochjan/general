...
class som:
    ...
    def __init__(self,x,y,inputs,eta_b=0.3,eta_n=0.1,nSize=0.5,alpha=1,usePCA=1,useBCs=0,eta_bfinal=0.03,eta_nfinal=0.01,nSizefinal=0.05):
        ...
        self.clusterSize = zeros(self.x*self.y)
    ...
    def somfwd(self,inputs, clusterSizeFlag=False):
        activations = sum((transpose(tile(inputs,(self.x*self.y,1)))-self.weights)**2,axis=0)
        best = argmin(activations)
        if clusterSizeFlag:
            tmp = self.clusterSize
            zscore = (tmp[best] - mean(tmp))/std(tmp)
            return best, activations, self.clusterSize[best], zscore, mean(tmp), std(tmp)
        else:
