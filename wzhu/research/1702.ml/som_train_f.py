...
def run(date, only_country=None):
    ...
    # add prev 3 weekly returns also
    week_ret_cols = [x for x in data.columns if x.endswith('.sum')]
    for col in week_ret_cols:
        for prevW in [1,2,3]:
            data[('p{}_'+col).format(prevW)] = data[col].shift(prevW)
    data = data.dropna()
