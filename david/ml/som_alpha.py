...
def make_country(data):
    ...
    data = pandas.pivot_table(data, rows=['BARRID', 'date'], cols='new_col', values=0)
    data = data.reset_index()
...
def run(date):
    ...
    for dt in dts:
        try:
            _tmp = pandas.read_csv('input_data/%s.csv' % dt.strftime('%Y%m%d'))
    ...
    nu.write_alpha_files(today, 'som_test', date)
